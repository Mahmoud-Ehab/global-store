...
The request body is prescribed by the implemented Builder classes; each class specifies the body structure (type). Generally, the "Builder.build" method builds Request objects out of Endpoints, and stores each Request, in a public object according to its method type (or may all of them be stored in the same object, say called "requests"), with a specified name. The body of the request, however, shall not be created by the build method, as it ought to be specified by the user. Perhaps the best way to address this issue is to define funtion for each possible request, where each function declares the required data (body) for the corresponding request. Of course, it's not just the best way, it is the most tedious as well. Despite that, it'll play a significant role in maintaining the application, by gathering (declaring) the constrains (the functions) in one place, rather than being dispersed in every piece of code that uses RequestDispatcher.
